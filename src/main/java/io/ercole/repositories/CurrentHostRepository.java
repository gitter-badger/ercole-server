...
public interface CurrentHostRepository extends PagingAndSortingRepository<CurrentHost, Long> {
	...
	@Query(nativeQuery = true, value =
		...
		+ " m.host_type, m.associated_cluster_name, NULL as extra_info from current_host m WHERE "
		+ " m.updated <= :date AND "
		+ " ("
	...
	@Query(nativeQuery = true, value = "WITH host_info AS (SELECT ch.hostname, "
		...
		+ "sum( CASE WHEN vfhi.license_count > 0 THEN vfhi.cpu*0.25 ELSE 0 END ) AS license_count FROM "
	...
	@Query(nativeQuery = true, value = "WITH host_info AS (SELECT ch.hostname, "
		...
		+ "vfhi.license_count > 0 THEN vfhi.cpu*0.25 ELSE 0 END ) AS license_count FROM " 
		+ "virtual_featured_host_info vfhi GROUP BY license_name), partial_summed_featured AS (SELECT "
	...
	@Query(nativeQuery = true, value = "WITH host_info AS (SELECT ch.hostname,"
		...
		+ "THEN vfhi.cpu*0.25 ELSE 0 END ) AS license_count FROM virtual_featured_host_info vfhi GROUP BY "
	...
	@Query(nativeQuery = true, value = ""
		...
		+ "		hb.dbName,"
		+ "		hb.hostname,"
		+ "		CAST((segmentAdvisor->>'Reclaimable') AS real) AS reclaimable"
		+ "	FROM"
		+ "		host_database hb,"
		+ "		jsonb_array_elements(segmentAdvisors) AS segmentAdvisor"
		+ "	WHERE"
		+ "		jsonb_typeof(segmentAdvisor->'Reclaimable') = 'number'"
